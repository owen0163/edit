...
app.get('/bills', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT b.id AS bill_id, b.user_id, u.name AS user_name, b.bill_date, b.total_amount, 
             bp.product_id, bp.quantity, bp.price, 
             p.name AS product_name, p.defaultprice AS product_defaultprice, 
             p.currentprice AS product_currentprice
      FROM bill b
    ...
    const bills = result.rows.reduce((acc, row) => {
      const { bill_id, user_id, user_name, bill_date, total_amount, product_id, quantity, price, product_name, product_defaultprice, product_currentprice } = row;
