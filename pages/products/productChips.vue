<template>
    <v-container fluid>
        <v-slide-group show-arrows class="pa-4" selected-class="bg-success" v-model="selectedProduct">
            <v-slide-item v-for="product in productChips" :key="product.id" class="mx-2">
                <v-card class="mx-auto" max-width="200">
                    ...
                    <v-card-actions class="d-flex justify-center">
                        <v-btn variant="elevated" color="blue-darken-2"
                            @click="addProduct(product.id, product.currentprice)">
                            ເພີ້ມສິນຄ້າ
                        </v-btn>
                    </v-card-actions>
                            ...
                            <template v-slot:default="{ isActive }">
                                <v-card title="Delete">
                                    <v-card-text class="d-flex justify-center">
                                        <v-col cols="12" sm="6">
                                            <v-text-field v-model="deleteInput" label="Please Input [DELETE]"
                                                variant="outlined" :rules="[rules.deleteproduct]">
                                            </v-text-field>
                                        </v-col>
                                    </v-card-text>
                                    <v-card-actions>
                                        <v-spacer></v-spacer>
                                        <v-btn variant="elevated" text="Close" @click="isActive.value = false"
                                            color="blue-darken-3">
                                            Close
                                        </v-btn>
                                        <v-btn variant="elevated" @click="submitForm(product.id, isActive)"
                                            color="red-darken-2">
                                            ລົບສິນຄ້າ
                                        </v-btn>
                                    </v-card-actions>
